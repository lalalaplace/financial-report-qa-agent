...
- `llm_insight` 只基于查询结果、确定性分析结果和主答案生成补充洞察。
- `assemble_final_answer` 只负责拼接有效补充洞察。
...
所以，LLM 只是链路中的规划、语言理解和补充表达组件。系统真正的执行能力被限制在可枚举 intent、可校验 QueryPlan、固定 SQL 模板、安全审查和只读数据库执行器之内。
