...
LLM 在系统中只承担有限职责：理解自然语言、生成结构化计划、判断多轮上下文关系、抽取澄清补丁。LLM 不直接操作数据库，不直接执行 SQL，不直接决定最终财务数值。
...
Agent 的装配入口是 `agent/graph.py`。系统用 `StateGraph(AgentState)` 把每个阶段拆成显式节点，并通过 `agent/routing.py` 做条件路由。

主链路如下：

```text
context_router
  -> llm_plan_query 或 clarification_patch 或 followup_patch
  ...
  -> analyze_*
  -> generate_answer / generate_derived_answer
...
- `llm_plan_query` 只生成 QueryPlan。
- `resolve_company` 只做公司标准化。
- `map_metric` 只做指标映射。
- `check_slots` 只做准入校验和澄清判断。
- `generate_*_sql` 只按固定 intent 生成对应 SQL。
- `review_and_execute_sql` 只审查和执行 SQL。
- `analyze_*` 只把 SQL 结果整理成业务结论。
- `generate_answer` 只基于已查询结果生成回答。
...
5. SQL 边界明确：SQL 由固定节点生成，不开放任意 SQL 问答。
...
9. 可追溯边界明确：PDF pipeline 有 `run_id`、运行日志、run history、lineage 和质量校验表。

所以，LLM 只是链路中的一个规划和语言理解组件。系统真正的执行能力被限制在可枚举 intent、可校验 QueryPlan、固定 SQL 模板、安全审查和只读数据库执行器之内。
