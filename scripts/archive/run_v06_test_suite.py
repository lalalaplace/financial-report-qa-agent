...
可通过 --include-legacy 串行运行 V0.4.5 与 V0.5 既有回归。
"""
...
TEST_GROUPS: dict[str, list[str]] = {
    ...
    "v060_clarification": [
        ...
        "scripts/agent/test_v060_clarification_schema.py",
    ],
...
LEGACY_COMMANDS: dict[str, list[str]] = {
    "v045_regression": ["scripts/agent/run_v045_test_suite.py"],
    "v05_regression": ["scripts/agent/run_v05_test_suite.py"],
}
...
def main() -> int:
    ...
    if args.list:
        for group_name, targets in TEST_GROUPS.items():
            ...
            for target in targets:
                print(f"  - {target}")
        for group_name, command in LEGACY_COMMANDS.items():
            print(f"{group_name}:")
    ...
    if args.include_legacy:
        for group_name, command in LEGACY_COMMANDS.items():
            if not _run_script(group_name, command):
                failed.append(group_name)
